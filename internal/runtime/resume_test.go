...
func TestResumeUsesFreshCacheGeneration(t *testing.T) {
	srv := natstest.Start(t)
	ctx := context.Background()
	...
	kv, err := srv.JS.CreateKeyValue(ctx, jetstream.KeyValueConfig{Bucket: "resume-cache-generation"})
	if err != nil {
	...
	eng, err := New(invoker.NewCache(kv, inv), st, sch, testSignals(t, st), map[string]*dsl.Flow{flow.Name: flow}, Config{})
	if err != nil {
		t.Fatalf("engine: %v", err)
	}
	...
	first := waitFor(id, store.StatusFailed)
	if first.NodeGeneration == 0 {
		t.Fatal("failed execution has generation 0, want cache-scoped generation")
	}
	...
	resumed := waitFor(id, store.StatusCompleted)
	if resumed.NodeGeneration <= first.NodeGeneration {
		t.Fatalf("generation after resume = %d, want > %d", resumed.NodeGeneration, first.NodeGeneration)
	}
	...
	if got := parseCtx(t, doc); string(got.Results["a"]) != `{"done":true}` {
		t.Fatalf("results.a = %s, want resumed success (not cached failure)", got.Results["a"])
	}
}
